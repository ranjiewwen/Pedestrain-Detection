...
const size_t fixed_size  = 4096/sizeof(int) + 8;
static const float atan2_p1 = 0.9997878412794807f*(float)(180/myCV_PI);
static const float atan2_p3 = -0.3258083974640975f*(float)(180/myCV_PI);
static const float atan2_p5 = 0.1555786518463281f*(float)(180/myCV_PI);
static const float atan2_p7 = -0.04432655554792128f*(float)(180/myCV_PI);
...
void createMat(myMat* img, size_* sz, int _channels, int _type, void* refdata)
{
	...
	img->step = img->channels * elesz * img->cols;
	img->totalsize = img->step*img->rows;
	if(!refdata)
	{
		img->data = (uchar*)malloc(img->totalsize);
		img->dataend = (uchar*)img->data + img->totalsize;
	}
	else
	{
		img->data = (uchar*)refdata;
		img->dataend = (uchar*)refdata + img->totalsize;
	}
};

__inline int myround (double val)
{
...
{
	int i;	
	sbresize(*vd,sbsize(*vs));
	for(i=0; i<sbcount(*vs); i++)
	{
		(*vd)[i] = (*vs)[i];
		sbcount_inc(*vd);
	}
...
void ABalc (AB_ *abobj, size_t _size, void* _type, int type)
{
	...
	if(_size > fixed_size)
	{
		if(type == 0)
		{
			abobj->ptr = calloc(_size,sizeof(int));
			abobj->size = _size;
		}
		else if (type == 1)
		{
			abobj->ptr = calloc(_size,sizeof(float));
			abobj->size = _size;
		}
		else
		{
			abobj->ptr = calloc(_size,sizeof(uchar));
			abobj->size = _size;
		}
	}
};
...
void nmitalc (NMIt_* obj, const myMat** _arrays, uchar** _ptrs)
{
    int i, d1=0, i0 = -1, d = -1;
    obj->arrays = _arrays;
    obj->ptrs = _ptrs;
    ...
    obj->nplanes = 0;
    obj->size = 0;
	myCV_Assert( (bool)(_arrays && (_ptrs || 0)) );
    ...
    if( obj->narrays < 0 )
    {
        for( i = 0; _arrays[i] != 0; i++ )
            ;
        obj->narrays = i;
        myCV_Assert((bool)(obj->narrays <= 1000));
    }

    obj->iterdepth = 0;

    for( i = 0; i < obj->narrays; i++ )
    {        
        const myMat* A = obj->arrays[i];
        if( obj->ptrs )
            obj->ptrs[i] = A->data;
		myCV_Assert((bool)(obj->arrays[i] != 0));
		...
		break;
            ...
            myCV_Assert((bool)((A->height == ((myMat*)obj->arrays[1])->height) && (A->width == ((myMat*)obj->arrays[1])->width)));
    }

    if( i0 >= 0 )
    {
        double total1;
		obj->size = ((myMat*)obj->arrays[i0])->width;
		total1 = (double)obj->size*((myMat*)obj->arrays[i0])->height;
        obj->size = (int)total1;
        obj->iterdepth = 0;
        obj->nplanes = 1;
    }
...
void cartToPolar( myMat* src1, myMat* src2, myMat* dst1, myMat* dst2, bool angleInDegrees )
{
    ...
    size_ tmp;
	myMat X = *src1, Y = *src2, Mag, Angle;
	const myMat* arrays[] = {&X, &Y, &Mag, &Angle, 0};
    int type = X.type, depth = X.depth, cn = X.channels, j, total, blockSize;
	uchar* ptrs[4];
	NMIt_ it;
	size_t esz1, i;

    myCV_Assert( (bool)(X.height == Y.height && X.width == Y.width && type == Y.type && (depth == myCV_32F || depth == myCV_64F)));
	type = myCV_MAT_TYPE(type);
	tmp.height = X.height; tmp.width = X.width;
	Mag = *dst1;	
	Angle = *dst2;	
    
	nmitalc(&it, arrays, ptrs);
    total = (int)(it.size*cn); blockSize = mymin(total, ((BLOCK_SIZE+cn-1)/cn)*cn);
    esz1 = 4;				
...
{
	float* hist = _hist;
	size_t i, sz = cache->blockHistogramSize;
	float sum = 0, scale, thresh;

	for( i = 0; i < sz; i++ )
       sum += hist[i]*hist[i];
    scale = 1.f/(sqrt(sum)+sz*0.1f); thresh = (float)descriptor->L2HysThreshold;
	for( i = 0, sum = 0; i < sz; i++ )
    {
        hist[i] = mymin(hist[i]*scale, thresh);
        sum += hist[i]*hist[i];
    }
...
{
    double delta = eps*(mymin(r1->width, r2->width) + mymin(r1->height, r2->height))*0.5;
    return fabs(r1->x - r2->x) <= delta &&
    fabs(r1->y - r2->y) <= delta &&
    fabs(r1->x + r1->width - r2->x - r2->width) <= delta &&
    fabs(r1->y + r1->height - r2->y - r2->height) <= delta;
};

int mypartition(rect_* _vec, int** labels, double eps)
{
	int i, j, N = (int)sbcount(_vec), nclasses = 0;
    const rect_* vec = _vec;
    const int PARENT=0;
    const int RANK=1;
	int* _nodes=NULL;
	int (*nodes)[2];
    ...
    for( i = 0; i < N; i++ )
    {
        ...
        for( j = 0; j < N; j++ )
        {
            ...
            {
                // unite both trees
                int rank = nodes[root][RANK], rank2 = nodes[root2][RANK], k, parent;
                if( rank > rank2 )
                ...
                myCV_Assert( (bool)(nodes[root][PARENT] < 0) );         
				k = j;
...
{
    int *labels=NULL, *rweights=NULL, *rejectLevels=NULL, nclasses, i, j, nlabels;
	double *rejectWeights=NULL;
	rect_ *rrects=NULL, tmp;
	if( groupThreshold <= 0 || sbcount(*rectList) == 0 )
    {
        if( weights )
        {
            int i, sz = sbcount(*rectList); //size_t  ->int
            sbresize(weights,sz);
            for( i = 0; i < sz; i++ )
                weights[i] = 1;
        }
    ...
    nclasses = mypartition(*rectList, &labels, eps);
	//below are confirmly suitable for img size 320 x 240
	tmp.x=0; tmp.y=0; tmp.width=0; tmp.height=0;
	for (i=0; i<nclasses; i++)
	{
		sbpush(rrects,tmp);
		sbpush(rweights,0);
		sbpush(rejectLevels,0);
		sbpush(rejectWeights,DBL_MIN);
	}
    nlabels = (int)sbcount(labels);
    for( i = 0; i < nlabels; i++ )
    {
        int cls = labels[i];
    ...
    if ( levelWeights && weights && !sbcount(weights) && !sbcount(levelWeights) )
    {
        for( i = 0; i < nlabels; i++ )
        {
            int cls = labels[i];
...
int myborderInterpolate( int p, int len, int borderType )
{
    if( (unsigned)p < (unsigned)len )
        ;
    else if( borderType == myBORDER_REPLICATE )
        p = p < 0 ? 0 : len - 1;
    else if( borderType == myBORDER_REFLECT || borderType == myBORDER_REFLECT_101 )
    {
        int delta = borderType == myBORDER_REFLECT_101;
        if( len == 1 )
            return 0;
        do
        {
    ...
    else if( borderType == myBORDER_WRAP )
    {
        if( p < 0 )
            p -= ((p-len+1)/len)*len;
        if( p >= len )
    ...
    else if( borderType == myBORDER_CONSTANT )
        p = -1;
    else
        exit(BORDER_INTERPOLATE_ERROR);
    return p;
};
...
const float* getBlock(hog_* descriptor, HCache_* cache, point_ pt, float* buf)
{
	...
	const PData_* _pixData;
	size_ blockSize = descriptor->blockSize;
    myCV_Assert((bool)(descriptor != 0));    
    pt.x += cache->imgoffset.x;
	pt.y += cache->imgoffset.y;

	if(!( (unsigned)pt.x <= (unsigned)(cache->grad.cols - blockSize.width) &&
          (unsigned)pt.y <= (unsigned)(cache->grad.rows - blockSize.height) ))
		exit(GET_BLOCK_FUNC_ERROR);
    if( cache->useCache )
    {
        point_ cacheIdx;
        uchar* computedFlag;
		if(!( pt.x % (int)cache->cacheStride.width == 0 && pt.y % (int)cache->cacheStride.height == 0 ))
			exit(GET_BLOCK_FUNC_ERROR);
        cacheIdx.x = pt.x/cache->cacheStride.width;
        cacheIdx.y = (int)(pt.y/cache->cacheStride.height) % (int)(cache->blockCache.rows);
        if( pt.y != cache->ymaxCached[cacheIdx.y] )
        {
			myMat cacheRow = cache->blockCacheFlags;
			uchar* ptr;
			cacheRow.rows = 1;
			cacheRow.data += cacheRow.step*cacheIdx.y;
			ptr = cacheRow.data;
			for(;ptr<cacheRow.dataend;ptr++)
				*ptr=0;
            cache->ymaxCached[cacheIdx.y] = pt.y;
        }

		blockHist = &(cache->blockCache.dataf + cache->blockCache.cols*cacheIdx.y)[cacheIdx.x*cache->blockHistogramSize];
		computedFlag = &(cache->blockCacheFlags.data + cache->blockCacheFlags.step*cacheIdx.y)[cacheIdx.x];
        if( *computedFlag != 0 )
            return blockHist;
        *computedFlag = (uchar)1; // set it at once, before actual computing
    }

    C1 = cache->count1; C2 = cache->count2; C4 = cache->count4;
    gradPtr = (const float*)(cache->grad.data + cache->grad.step*pt.y) + pt.x*2;
    qanglePtr = cache->qangle.data + cache->qangle.step*pt.y + pt.x*2;

    if(!( blockHist != 0 ))
		exit(GET_BLOCK_FUNC_ERROR);
	for( k = 0; k < cache->blockHistogramSize; k++ )
        blockHist[k] = 0.f;

    _pixData = &cache->pixData[0];

    for( k = 0; k < C1; k++ )
    {
        const PData_* pk = &_pixData[k];
    ...
    normalizeBlockHistogram(descriptor, cache, blockHist);
...
{
    size_ gradsize, wholeSize, tmp;
	point_ roiofs;
	ptrdiff_t delta1, delta2;
	int minstep, i, x, y, cn, width, _nbins;
	float angleScale;
	myMat _lut, Dx, Dy, Mag, Angle;
	const float* lut;
	float *_dbuf_buf, *dbuf;
	AB_ mapbuf, _dbuf;
	int *mapbuf_buf, *xmap, *ymap;
	const int borderType = (int)myBORDER_REFLECT_101;

	if(!( img->type == myCV_8U || img->type == myCV_8UC3 ))
	{
		printf("\n ERROR IN IMAGE_TYPE!");
		exit(IMAGE_TYPE_ERROR);
	}
	
	gradsize.width = img->cols + paddingTL.width + paddingBR.width;
	gradsize.height = img->rows + paddingTL.height + paddingBR.height;
	createMat(grad, &gradsize, 2, myCV_32F, 0);
	createMat(qangle, &gradsize, 2, 0, 0);	
	delta1 = img->data - img->datastart;
	delta2 = img->dataend - img->datastart;

	if(!delta1)
		roiofs.x = roiofs.y = 0;
	else
	{
		roiofs.y = (int)(delta1/img->step);
		roiofs.x = (int)(delta1 - img->step*roiofs.y);
	}
	minstep = (roiofs.x + img->cols)*img->channels;
	wholeSize.height = (int)((delta2 - minstep)/img->step + 1);
	wholeSize.height = mymax(wholeSize.height, roiofs.y + img->rows);
	wholeSize.width = (int)((delta2 - img->step*(wholeSize.height-1))/img->channels);
	wholeSize.width = mymax(wholeSize.width, roiofs.x + img->cols);
	
    cn = img->channels;
	tmp.height = 1; tmp.width = 256;
	createMat(&_lut, &tmp, 1, myCV_32F, 0);
	lut = &(_lut.dataf)[0];

	if( obj->gammaCorrection )
        for( i = 0; i < 256; i++ )
            (_lut.dataf)[i] = sqrt((float)i);
	else
        for( i = 0; i < 256; i++ )
            (_lut.dataf)[i] = (float)i;

	mapbuf.buffer_pad = buffer_padding;
	mapbuf_buf = (int*)calloc(mapbuf.buffer_pad + fixed_size,sizeof(int));
    ABalc(&mapbuf, (gradsize.width + gradsize.height + 4), mapbuf_buf, 0);
    xmap = (int*)mapbuf.ptr + 1;
    ymap = xmap + (int)gradsize.width + 2;
    for( x = -1; x < gradsize.width + 1; x++ )
        xmap[x] = myborderInterpolate(x - paddingTL.width + roiofs.x,									
                        wholeSize.width, borderType) - roiofs.x;
    for( y = -1; y < gradsize.height + 1; y++ )
        ymap[y] = myborderInterpolate(y - paddingTL.height + roiofs.y,								
                        wholeSize.height, borderType) - roiofs.y;

    width = gradsize.width;
   	_dbuf.buffer_pad = buffer_padding;
	_dbuf_buf = (float*)calloc(_dbuf.buffer_pad + fixed_size,sizeof(float));
	ABalc(&_dbuf, width*4, _dbuf_buf, 1);
	dbuf = (float*)_dbuf.ptr;
	tmp.height = 1;
	tmp.width = width;
	createMat(&Dx, &tmp, 1, myCV_32F, dbuf);
	createMat(&Dy, &tmp, 1, myCV_32F, dbuf + width);
	createMat(&Mag, &tmp, 1, myCV_32F, dbuf + width*2);
	createMat(&Angle, &tmp, 1, myCV_32F, dbuf + width*3);

    _nbins = obj->nbins;
    angleScale = (float)(_nbins/myCV_PI);
	...
	for( y = 0; y < gradsize.height; y++ )
    {
        ...
        for( x = 0; x < width; x++ )
        {
            ...
            if( hidx < 0 )
                ...
                hidx -= _nbins;
            myCV_Assert( (bool)((unsigned)hidx < (unsigned)_nbins) );
...
{
    size_ blockSize, blockStride, cellSize, nblocks, ncells;
	int i, j, nbins, rawBlockSize;
	myMat weights;
	float sigma, scale;

	computeGradient(descriptor, img, &cache->grad, &cache->qangle, paddingTL, paddingBR);
	cache->imgoffset.x = (int)paddingTL.width;
	cache->imgoffset.y = (int)paddingTL.height;

	cache->cacheStride = cacheStride;
	cache->useCache = useCache;
    cache->winSize = descriptor->winSize;
	cache->pixData = NULL;
	cache->blockData = NULL;
	cache->ymaxCached = NULL;
    blockSize = descriptor->blockSize;
    blockStride = descriptor->blockStride;
    cellSize = descriptor->cellSize;
    nbins = descriptor->nbins;
    rawBlockSize = blockSize.width*blockSize.height;

	sizealc(&nblocks, (cache->winSize.width - blockSize.width)/blockStride.width + 1, (cache->winSize.height - blockSize.height)/blockStride.height + 1);
	sizealc(&ncells, blockSize.width/cellSize.width, blockSize.height/cellSize.height);
	cache->nblocks = nblocks;
	cache->ncells = ncells;
	cache->blockHistogramSize = ncells.width*ncells.height*nbins;
	
    if( useCache )
    {
        size_ cacheSize,tmp;
		int cacheRows, ii; //size_t  ->t
		sizealc(&cacheSize, (int)((cache->grad.cols - blockSize.width)/cacheStride.width+1), (int)(cache->winSize.height/cacheStride.height+1));
		tmp.width = cacheSize.width*cache->blockHistogramSize;
		tmp.height = cacheSize.height;
		createMat(&cache->blockCache, &tmp, 1, myCV_32F, 0);													
		createMat(&cache->blockCacheFlags, &cacheSize, 1, 0, 0);
        cacheRows = cache->blockCache.rows;
        sbresize(cache->ymaxCached,cacheRows);
        for(ii = 0; ii < cacheRows; ii++ )
            cache->ymaxCached[ii] = -1;
	}
	createMat(&weights, &blockSize, 1, myCV_32F, 0);
	if (descriptor->winSigma >= 0)
		sigma = (float)descriptor->winSigma;
	else
		sigma = (float)(blockSize.width + blockSize.height)/8;
    scale = 1.f/(sigma*sigma*2);
    ...
    for(i = 0; i < blockSize.height; i++)
        for(j = 0; j < blockSize.width; j++)
        {
			...
			(weights.dataf + weights.cols*i)[j] = exp(-(di*di + dj*dj)*scale);
        }

    sbresize(cache->blockData,(nblocks.width*nblocks.height));
    sbresize(cache->pixData,(rawBlockSize*3));
    cache->count1 = cache->count2 = cache->count4 = 0; //count1,count2,count4分别表示block中同时对1个cell，2个cell，4个cell有贡献的像素点的个数。
    for( j = 0; j < blockSize.width; j++ )
        for( i = 0; i < blockSize.height; i++ )
        {
            PData_* data = 0;
            ...
            int icellX0 = floor(cellX);
            int icellY0 = floor(cellY);
                ...
                (unsigned)icellX1 < (unsigned)ncells.width )
            {
                ...
                {
                    data = &cache->pixData[rawBlockSize*2 + (cache->count4++)];
                ...
                {
                    data = &cache->pixData[rawBlockSize + (cache->count2++)];
                    if( (unsigned)icellY0 < (unsigned)ncells.height )
                    {
                        icellY1 = icellY0;
                        cellY = 1.f - cellY;
            ...
            else
            {
                ...
                {
                    data = &cache->pixData[cache->count1++];
                    if( (unsigned)icellY0 < (unsigned)ncells.height )
                    {
                        icellY1 = icellY0;
                        cellY = 1.f - cellY;
            ...
            data->gradOfs = (cache->grad.cols*i + j)*2;
            data->qangleOfs = (cache->qangle.cols*i + j)*2;
            data->gradWeight = (weights.dataf + weights.cols*i)[j];
        }

    myCV_Assert( (bool)(cache->count1 + cache->count2 + cache->count4 == rawBlockSize) );
    // defragment pixData
    for( j = 0; j < cache->count2; j++ )
        cache->pixData[j + cache->count1] = cache->pixData[j + rawBlockSize];
    for( j = 0; j < cache->count4; j++ )
        cache->pixData[j + cache->count1 + cache->count2] = cache->pixData[j + rawBlockSize*2];
    cache->count2 += cache->count1;
    cache->count4 += cache->count2;

    // initialize blockData
    for( j = 0; j < nblocks.width; j++ )
        for( i = 0; i < nblocks.height; i++ )
        {
            BData_* data = &cache->blockData[(int)(j*nblocks.height + i)];
            data->histOfs = (j*nblocks.height + i)*cache->blockHistogramSize;
            data->imgOffset.x = j*blockStride.width;
			data->imgOffset.y = i*blockStride.height;
        }
	free(weights.data);
};
...
{
	HCache_ cache;
	size_ cacheStride, paddedImgSize;
	size_t dsize, i;
	double rho;
	int nwindows = 0, nblocks, blockHistogramSize;
	BData_* blockData;
	sbfree(*hits);	
    sizealc(&cacheStride, gcd(winStride.width, obj->blockStride.width), gcd(winStride.height, obj->blockStride.height)); 
	
    padding.width = (int)myalignSize(mymax(padding.width, 0), cacheStride.width);
    padding.height = (int)myalignSize(mymax(padding.height, 0), cacheStride.height);
	
	sizealc(&paddedImgSize, (img->cols + padding.width*2), (img->rows + padding.height*2));

    cacheinit(obj, &cache, img, padding, padding, nwindows == 0, cacheStride);  //初始化，计算梯度

	nblocks = cache.nblocks.width * cache.nblocks.height;
    blockHistogramSize = cache.blockHistogramSize;
	blockData = &cache.blockData[0];

    if( !nwindows )
	{
		nwindows = (((int)((paddedImgSize.width - cache.winSize.width)/winStride.width)+1) * ((int)((paddedImgSize.height - cache.winSize.height)/winStride.height)+1));
	}

	if (!((int)obj->blockSize.width % (int)obj->cellSize.width == 0 && (int)obj->blockSize.height % (int)obj->cellSize.height == 0 &&
		(int)(cache.winSize.width - obj->blockSize.width) % (int)obj->blockStride.width == 0 &&
		(int)(cache.winSize.height - obj->blockSize.height) % (int)obj->blockStride.height == 0))
		exit(DESCRIPTOR_SIZE_ERROR);

	dsize = obj->nbins * (obj->blockSize.width/obj->cellSize.width) * (obj->blockSize.height/obj->cellSize.height) *
					((cache.winSize.width - obj->blockSize.width)/obj->blockStride.width + 1) * 
					((cache.winSize.height - obj->blockSize.height)/obj->blockStride.height + 1);

    rho = obj->svmSize > dsize ? obj->svmDetector[dsize] : 0;
	
	for( i = 0; i < nwindows; i++ )
    {
        point_ pt0;
		double s = rho;
        const float* svmVec = obj->svmDetector;
        int j, k, x, y, nwindowsX;
		nwindowsX = (paddedImgSize.width - cache.winSize.width) / winStride.width + 1;
		y = (int)(i) / nwindowsX;
		x = (int)(i) - nwindowsX * y;
		pt0.x = x*winStride.width - padding.width;
		pt0.y = y*winStride.height - padding.height;
        if(!(pt0.x % (int)cacheStride.width == 0 && pt0.y % (int)cacheStride.height == 0))
			exit(DETECT_FUNC_ERROR);
        
        
        for( j = 0; j < nblocks; j++, svmVec += blockHistogramSize )  //105*36=3780
        {
            const float* vec;
			BData_* bj = &cache.blockData[j];
			point_ pt;
            pt.x = pt0.x + bj->imgOffset.x;
			pt.y = pt0.y + bj->imgOffset.y;      
			vec = getBlock(obj, &cache, pt, NULL);
	...
	sbfree(cache.pixData);
	sbfree(cache.blockData);
	sbfree(cache.ymaxCached);
	free(cache.grad.data);
	free(cache.qangle.data);
	free(cache.blockCache.data);
	free(cache.blockCacheFlags.data);
};

void vresize(WT** src, T* dst, AT* beta, int width )
{
	AT b0 = beta[0], b1 = beta[1];
...
{
	size_ ssize, dsize;
	int cn = src->channels, dy, bufstep, k;
	AB_ _buffer;
	WT* _buffer_buf;
	...
	dsize.width = dst->width;
	dsize.height = dst->height;
    ssize.width *= cn;
    dsize.width *= cn;
    xmin *= cn;
    xmax *= cn;

	bufstep = (int)myalignSize(dsize.width, 16);
	_buffer.buffer_pad = buffer_padding;
	_buffer_buf = (WT*)calloc(_buffer.buffer_pad + fixed_size, sizeof(WT));
	ABalc(&_buffer, bufstep*ksize, _buffer_buf, 1);

    for(k = 0; k < ksize; k++ )
    {
        prev_sy[k] = -1;
        rows[k] = (WT*)_buffer.ptr + bufstep*k;
    }
	...
	for( dy = 0; dy < dsize.height; dy++, beta += ksize )
    {
        ...
        for(k = 0; k < ksize; k++ )
        {
            ...
            if( k1 == ksize )
                k0 = mymin(k0, k); // remember the first row that needs to be computed
            srows[k] = (T*)(src->data + src->step*sy);
            prev_sy[k] = sy;
        }

        if( k0 < ksize )
            hresize( (T**)(srows + k0), (WT**)(rows + k0), ksize - k0, xofs, (AT*)(_alpha),
                    ssize.width, dsize.width, cn, xmin, xmax );
        vresize( (WT**)rows, (T*)(dst->data + dst->step*dy), beta, dsize.width );
    }
};
...
{
	int INTER_RESIZE_COEF_SCALE = 1 << 11, depth, cn, k, sx, sy, dx, dy, xmin, xmax, width, ksize, ksize2, v;
	myMat src = *_src, dst = *_dst;
    size_ ssize;
	int dsize_area = dsize.width*dsize.height;
	double scale_x, scale_y;
	bool area_mode, fixpt;
	float fx, fy, cbuf[MAX_ESIZE];
	AB_ _buffer;
	uchar* _buffer_buf;
	int *xofs, *yofs;
	float *alpha, *beta;
	...
	ssize.width = src.width;
	ssize.height = src.height;
    myCV_Assert((bool)((ssize.height*ssize.width) > 0));
	myCV_Assert( (bool)((dsize_area) || (inv_scale_x > 0 && inv_scale_y > 0)) );
    if( !dsize_area )
    {
		dsize.width = myround(src.cols*inv_scale_x);
		dsize.height = myround(src.rows*inv_scale_y);
		myCV_Assert( (bool)(dsize.width*dsize.height) );
    }
    else
    {
	...
	depth = src.depth; cn = src.channels;
    scale_x = 1./inv_scale_x; scale_y = 1./inv_scale_y;

    xmin = 0; xmax = dsize.width; width = dsize.width*cn;
    area_mode = interpolation == INTER_AREA;
    fixpt = depth == myCV_8U;
	ksize=2;
    ksize2 = ksize/2;
	_buffer.buffer_pad = buffer_padding;
	_buffer_buf = (uchar*)calloc(_buffer.buffer_pad + fixed_size, sizeof(uchar));
	ABalc(&_buffer, (width + dsize.height)*(sizeof(int) + sizeof(float)*ksize), _buffer_buf, 2);

    xofs = (int*)(uchar*)_buffer.ptr;
    yofs = xofs + width;
    alpha = (float*)(yofs + (int)dsize.height);
    ialpha = (short*)alpha;
    beta = alpha + width*ksize;
    ibeta = ialpha + width*ksize;
    ...
    for( dx = 0; dx < dsize.width; dx++ )
    {
        ...
        if( fixpt )
        {
            for( k = 0; k < ksize; k++ )
			{
				v = myround(cbuf[k]*INTER_RESIZE_COEF_SCALE);
                ialpha[dx*cn*ksize + k] = (short)((unsigned)(v - SHRT_MIN) <= (unsigned)USHRT_MAX ? 
    ...
    for( dy = 0; dy < dsize.height; dy++ )
    {
        ...
        if( fixpt )
        {
			...
			{
				v = myround(cbuf[k]*INTER_RESIZE_COEF_SCALE);
                ibeta[dy*ksize + k] = (short)((unsigned)(v - SHRT_MIN) <= (unsigned)USHRT_MAX ? 
											v : v > 0 ? SHRT_MAX : SHRT_MIN);
			}
        }
        else
...
{
	int i, i1 = start, i2 = end;
    double minScale = i1 > 0 ? (*levelScale)[i1] : i2 > 1 ? (*levelScale)[i1+1] : mymax(img->cols, img->rows);
	size_ maxSz;
	myMat smallerImgBuf;
	point_* locations=NULL;		
    double* hitsWeights=NULL;	

	maxSz.width = ceil(img->cols/minScale);
	maxSz.height = ceil(img->rows/minScale);
	createMat(&smallerImgBuf, &maxSz, img->channels, img->type, 0);    	

    for( i = i1; i < i2; i++ )
    {
        double scale = (*levelScale)[i];
		size_ sz, scaledWinSize;
		myMat smallerImg;
		size_t j;

		sz.width = myround(img->cols/scale);
		sz.height = myround(img->rows/scale);														
       
		createMat(&smallerImg, &sz, img->channels, img->type, smallerImgBuf.data);
		if( sz.width == img->width && sz.height == img->height )
			createMat(&smallerImg, &sz, img->channels, img->type, img->data);
        else			
            resizeMat(img, &smallerImg, sz, 0.0, 0.0, 1);	

        detect(obj,&smallerImg, &locations, &hitsWeights, hitThreshold, winStride, padding);

		scaledWinSize.width = myround(obj->winSize.width*scale);
		scaledWinSize.height = myround(obj->winSize.height*scale);

        for( j = 0; j < sbcount(locations); j++ )
        {
            rect_ tmp;
			tmp.x = myround(locations[j].x*scale);
			tmp.y = myround(locations[j].y*scale);
			tmp.width = scaledWinSize.width;
			tmp.height = scaledWinSize.height;
			sbpush(*allCandidates,tmp);
			sbpush(*scales,scale);
        }
		for (j = 0; j < sbcount(locations); j++)
        {
            sbpush(*weights,hitsWeights[j]);
        }
		if (smallerImg.data != smallerImgBuf.data && smallerImg.data != img->data && (smallerImg.data))
			free(smallerImg.data);
    }
	free(smallerImgBuf.data);
	sbfree(hitsWeights);
};
					  ...
					  size_ winStride, size_ padding, double scale0, double finalThreshold, bool useMeanshiftGrouping)
{
	double scale = 1.;
    int levels = 0;	
	double* foundWeights=NULL;
    double* levelScale=NULL;		
    rect_* allCandidates=NULL;	
    double* tempScales=NULL;
    double* tempWeights=NULL;
    double* foundScales=NULL;

    for( levels = 0; levels < obj->nlevels; levels++ )
    {
        sbpush(levelScale,scale);
        if( myround(img->cols/scale) < obj->winSize.width ||
            myround(img->rows/scale) < obj->winSize.height ||
            scale0 <= 1 )
            break;
        scale *= scale0;
    }
    levels = mymax(levels, 1);
    sbresize(levelScale,levels);		
	parallel(0, sbcount(levelScale), obj, img, hitThreshold, winStride, padding, &levelScale, &allCandidates, &tempWeights, &tempScales);
	veccopydouble(&tempScales, &foundScales);   //将tempScales中的内容复制到foundScales中

	/*printf("\ntmpscale %d, foundscale %d", sbcount(tempScales), sbcount(foundScales));
	for (levels = 0; levels < sbcount(tempScales); levels++)
		printf("\n%f", foundScales[levels]);
    */
	veccopyrect_(&allCandidates, foundLocations);  //将候选目标窗口保存在foundLocations中
	
	/*printf("\nallCand %d found %d", sbcount(allCandidates), sbcount(*foundLocations));
	for (levels=0; levels<sbcount(allCandidates); levels++)
		printf("\n\n x=%d y=%d w=%d h=%d",allCandidates[levels].x,allCandidates[levels].y, allCandidates[levels].width,allCandidates[levels].height);*/
	
	veccopydouble(&tempWeights, &foundWeights);    //将候选目标可信度保存在foundWeights中
    groupRectangles(foundLocations, (int)finalThreshold, 0.2, 0, 0);  //对矩形框进行聚类
	sbfree(foundWeights);
	sbfree(levelScale);
	sbfree(allCandidates);
	sbfree(tempScales);
	sbfree(tempWeights);
	sbfree(foundScales);
};
...
{	
	...
	obj->derivAperture = 1;
	obj->winSigma = -1;
	obj->histogramNormType = 0;
	obj->L2HysThreshold = 0.2;
